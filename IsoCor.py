...
## Updated for Python 3 by conversion script
...
class gui:
    def __init__(self):
        ...
        # create main frame
        app = wx.App()
        self.frame = wx.Frame(None, -1, title='IsoCor', size=(900, 600))
        panel = wx.Panel(self.frame)
        
        # Usando GridBagSizer em vez de RowColSizer (compatível com Python 3)
        sizer = wx.GridBagSizer(5, 5)
        
        # load main window if isotopic data loaded
        if self.exist('Isotopes.dat'):
            self.isotop = self.load_iso('Isotopes.dat')
            if self.isotop:
                # create widgets
                self.dict_form_meta, self.meta_list = self.load_db('Metabolites.dat')
                self.dict_form_der, self.der_list = self.load_db('Derivatives.dat')
                
                # Listbox de metabólitos
                self.listbox_metabolites = wx.ListBox(panel, -1, size=(180, 250), choices=self.meta_list, style=wx.LB_SINGLE)
                self.listbox_metabolites.Bind(wx.EVT_LISTBOX, self.select_meta)
                sizer.Add(self.listbox_metabolites, pos=(0, 0), span=(8, 1), flag=wx.EXPAND|wx.ALL, border=5)
                
                # Isotopic tracer
                lab_el_cor = wx.StaticText(panel, -1, "Isotopic tracer:")
                sizer.Add(lab_el_cor, pos=(0, 1), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                default = 'C' if 'C' in self.isotop else list(self.isotop.keys())[0]
                self.cb_el_cor = wx.ComboBox(panel, -1, size=(60, -1), value=default,
                                          choices=list(self.isotop.keys()), style=wx.CB_READONLY)
                self.cb_el_cor.Bind(wx.EVT_COMBOBOX, self.modif_el_cor)
                sizer.Add(self.cb_el_cor, pos=(0, 2), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                # Metabolite formula
                self.lab_form_meta = wx.StaticText(panel, -1, "Metabolite formula:")
                sizer.Add(self.lab_form_meta, pos=(1, 1), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                self.entry_form_meta = wx.TextCtrl(panel, -1, size=(150, -1))
                self.entry_form_meta.Bind(wx.EVT_TEXT, self.modif_form_meta)
                sizer.Add(self.entry_form_meta, pos=(1, 2), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                # Derivative formula
                lab_form_der = wx.StaticText(panel, -1, "Derivative formula:")
                sizer.Add(lab_form_der, pos=(2, 1), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                self.cb_der = wx.ComboBox(panel, -1, size=(100, -1), value='none',
                                          choices=['none'] + self.der_list + ['custom...'], style=wx.CB_READONLY)
                self.cb_der.Bind(wx.EVT_COMBOBOX, self.select_der)
                sizer.Add(self.cb_der, pos=(2, 2), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                self.entry_form_der = wx.TextCtrl(panel, -1, size=(150, -1))
                self.entry_form_der.Hide()
                self.entry_form_der.Bind(wx.EVT_TEXT, self.modif_form_der)
                sizer.Add(self.entry_form_der, pos=(2, 3), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                # Opções
                lab_opt = wx.StaticText(panel, -1, "Include correction for:")
                sizer.Add(lab_opt, pos=(3, 1), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                lab_el_excluded = wx.StaticText(panel, -1, "- nat. ab. of the tracer:")
                sizer.Add(lab_el_excluded, pos=(4, 1), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                self.cb_el_excluded = wx.ComboBox(panel, -1, size=(50, -1), value='yes',
                                          choices=["yes", "no"], style=wx.CB_READONLY)
                sizer.Add(self.cb_el_excluded, pos=(4, 2), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                lab_warning = wx.StaticText(panel, -1, "(please refer to the tutorial for details)")
                sizer.Add(lab_warning, pos=(5, 1), span=(1, 2), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                lab_mean_enr = wx.StaticText(panel, -1, "Calc. mean enrichment:")
                sizer.Add(lab_mean_enr, pos=(6, 1), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                self.cb_mean_enr = wx.ComboBox(panel, -1, size=(50, -1), value='yes',
                                          choices=["yes", "no"], style=wx.CB_READONLY)
                sizer.Add(self.cb_mean_enr, pos=(6, 2), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                lab_el_pur = wx.StaticText(panel, -1, "- purity of the tracer:")
                sizer.Add(lab_el_pur, pos=(7, 1), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                self.entry_el_pur = wx.TextCtrl(panel, -1, size=(150, -1))
                self.entry_el_pur.ChangeValue('0;'*(len(self.isotop[self.cb_el_cor.GetValue()])-1) + '1.0')
                sizer.Add(self.entry_el_pur, pos=(7, 2), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                # Botões
                cmd_process = wx.Button(panel, -1, 'Process...')
                cmd_process.Bind(wx.EVT_BUTTON, self.cmd_correction)
                sizer.Add(cmd_process, pos=(8, 1), span=(1, 2), flag=wx.ALIGN_CENTER|wx.ALL, border=5)
                
                cmd_load_single = wx.Button(panel, -1, 'Load single meas.')
                cmd_load_single.Bind(wx.EVT_BUTTON, self.cmd_parse_single)
                sizer.Add(cmd_load_single, pos=(0, 4), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                cmd_load_multiple = wx.Button(panel, -1, 'Load mult. meas.')
                cmd_load_multiple.Bind(wx.EVT_BUTTON, self.cmd_parse_multiple)
                sizer.Add(cmd_load_multiple, pos=(1, 4), flag=wx.ALIGN_CENTER_VERTICAL|wx.ALL, border=5)
                
                # Lista de dados
                self.liste = wx.ListBox(panel, -1, size=(120, 250))
                sizer.Add(self.liste, pos=(2, 4), span=(6, 1), flag=wx.EXPAND|wx.ALL, border=5)
                
                # Área de texto para resultados
                self.edit = wx.TextCtrl(panel, -1, style=wx.TE_MULTILINE|wx.TE_READONLY|wx.TE_WORDWRAP)
                sizer.Add(self.edit, pos=(9, 0), span=(1, 5), flag=wx.EXPAND|wx.ALL, border=5)
                
                # Configurar proporções de crescimento
