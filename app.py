...
def run_isocor(outputdataset, isotop, dict_form_meta, dict_form_der,
    ...
    for job in jobs:
        ...
        # warn if a derivative was specified but not found
        if der_name and der_name not in dict_form_der:
            rows.append({
                "sample": sample, "metabolite": meta_name,
                "derivative": der_name, "isotopologue": 0,
                "isotopologue_fraction": np.nan,
                "residuum": np.nan, "mean_enrichment": np.nan,
                "error": f"derivative '{der_name}' not found in Derivatives.dat — correction run without derivative",
            })
